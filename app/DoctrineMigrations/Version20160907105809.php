 ...
 */
class Version20160907105809 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql('DROP TABLE consumption_user');
        $this->addSql('DROP TABLE user_consumption');
        $this->addSql('DROP TABLE user_sleep');
        $this->addSql('CREATE TEMPORARY TABLE __temp__consumption AS SELECT id FROM consumption');
        $this->addSql('DROP TABLE consumption');
        $this->addSql('CREATE TABLE consumption (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, PRIMARY KEY(id), CONSTRAINT FK_2CFF2DF9A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO consumption (id) SELECT id FROM __temp__consumption');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TABLE consumption_user (consumption_id INTEGER NOT NULL, user_id INTEGER NOT NULL, PRIMARY KEY(consumption_id, user_id))');
        $this->addSql('CREATE INDEX IDX_4063BCDEA76ED395 ON consumption_user (user_id)');
        $this->addSql('CREATE INDEX IDX_4063BCDED17C3821 ON consumption_user (consumption_id)');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__consumption AS SELECT id FROM consumption');
        $this->addSql('DROP TABLE consumption');
        $this->addSql('CREATE TABLE consumption (id INTEGER NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO consumption (id) SELECT id FROM __temp__consumption');
        $this->addSql('DROP TABLE __temp__consumption');
        $this->addSql('DROP INDEX IDX_739D823BA76ED395');
        $this->addSql('DROP INDEX IDX_739D823BE934951A');
