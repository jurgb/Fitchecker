 ...
 */
class Version20160907130715 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TABLE equipment (id INTEGER NOT NULL, name VARCHAR(255) DEFAULT NULL, location VARCHAR(255) DEFAULT NULL, PRIMARY KEY(id))');
        $this->addSql('CREATE TABLE exercise_equipment (exercise_id INTEGER NOT NULL, equipment_id INTEGER NOT NULL, PRIMARY KEY(exercise_id, equipment_id))');
        $this->addSql('CREATE INDEX IDX_7DEFD369E934951A ON exercise_equipment (exercise_id)');
        $this->addSql('CREATE INDEX IDX_7DEFD369517FE9FE ON exercise_equipment (equipment_id)');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__sleep AS SELECT id, user_id, hours FROM sleep');
        $this->addSql('DROP TABLE sleep');
        $this->addSql('CREATE TABLE sleep (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, hours INTEGER NOT NULL, PRIMARY KEY(id), CONSTRAINT FK_F33C2ACA76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO sleep (id, user_id, hours) SELECT id, user_id, hours FROM __temp__sleep');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__sleep AS SELECT id, user_id, hours FROM sleep');
        $this->addSql('DROP TABLE sleep');
        $this->addSql('CREATE TABLE sleep (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, hours INTEGER NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO sleep (id, user_id, hours) SELECT id, user_id, hours FROM __temp__sleep');
        $this->addSql('DROP TABLE __temp__sleep');
        $this->addSql('CREATE INDEX IDX_F33C2ACA76ED395 ON sleep (user_id)');
        $this->addSql('DROP INDEX IDX_4E57311CE934951A');
